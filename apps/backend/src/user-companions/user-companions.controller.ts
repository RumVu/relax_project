...
export class UserCompanionsController {
  ...
  getMemoryInsights(@CurrentUser() user: AuthUser) {
    return this.userCompanionsService.getMemoryInsights(user.id);
  }

  @ApiOperation({ summary: 'Get weekly memory card from companion' })
  @ApiOkResponse({ description: 'Weekly summary card generated by companion.' })
  @UseGuards(JwtAuthGuard)
  @Get('me/memory/weekly')
  getWeeklyMemoryCard(@CurrentUser() user: AuthUser) {
    return this.userCompanionsService.getWeeklyMemoryCard(user.id);
  }
}
